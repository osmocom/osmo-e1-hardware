...
#define E1_TX_CR_LOOPBACK_CROSS	(1 <<  6)
...
void
e1_init(int chan, bool clk_mode)
{
	/* Global state init */
	memset(&g_e1[chan], 0x00, sizeof(g_e1[0]));

	/* Reset FIFOs */
#if 0
	e1f_reset(g_e1[chan].rx.fifo, (chan << 8) +   0, 128);
	e1f_reset(g_e1[chan].tx.fifo, (chan << 8) + 128, 128);
#else
	e1f_reset(&g_e1[chan].fifo, (chan << 8) +   0, 128);
	g_e1[chan].rx.fifo = &g_e1[chan].fifo;
	g_e1[chan].tx.fifo = &g_e1[chan].fifo;
#endif

	/* Enable Rx */
	g_e1[chan].rx.cr = E1_RX_CR_OVFL_CLR |
	                   E1_RX_CR_MODE_MFA |
	                   E1_RX_CR_ENABLE;
	e1_regs[chan].rx.csr = g_e1[chan].rx.cr;

	/* Enable Tx */
	g_e1[chan].tx.cr = E1_TX_CR_UNFL_CLR |
	                   (clk_mode ? E1_TX_CR_TICK_REMOTE : E1_TX_CR_TICK_LOCAL) |
	                   E1_TX_CR_MODE_TS0_CRC_E |
		           E1_TX_CR_ENABLE;
	e1_regs[chan].tx.csr = g_e1[chan].tx.cr;

	/* State */
	g_e1[chan].rx.state = BOOT;
	g_e1[chan].tx.state = BOOT;
}
...
unsigned int
e1_rx_need_data(int chan, unsigned int usb_addr, unsigned int max_frames)
{
	unsigned int ofs;
	int tot_frames = 0;
	int n_frames;
	...
	while (max_frames) {
		...
		n_frames = e1f_frame_read(g_e1[chan].rx.fifo, &ofs, max_frames);
		if (!n_frames)
			break;
...
unsigned int
e1_tx_feed_data(int chan, unsigned int usb_addr, unsigned int frames)
{
	unsigned int ofs;
	int n_frames;
	...
	while (frames) {
		...
		n_frames = e1f_frame_write(g_e1[chan].tx.fifo, &ofs, frames);
		if (!n_frames) {
...
unsigned int
e1_tx_level(int chan)
{
	return e1f_valid_frames(g_e1[chan].tx.fifo);
}

unsigned int
e1_rx_level(int chan)
{
	return e1f_valid_frames(g_e1[chan].rx.fifo);
}
...
e1_poll(int chan)
{
	...
	/* Active ? */
	if ((g_e1[chan].rx.state == IDLE) && (g_e1[chan].tx.state == IDLE))
		return;

	/* HACK: LED link status */
	if (!(e1_regs[chan].rx.csr & 2))
		e1_led_change(chan ? LED_CH1_GREEN : LED_CH0_GREEN, LED_MODE_OFF);
	else if (g_e1[chan].tx.state == RUN)
		e1_led_change(chan ? LED_CH1_GREEN : LED_CH0_GREEN, LED_MODE_FAST);
	else
		e1_led_change(chan ? LED_CH1_GREEN : LED_CH0_GREEN, LED_MODE_ON);

	/* Recover any done TX BD */
	while ( (bd = e1_regs[chan].tx.bd) & E1_BD_VALID ) {
		e1f_multiframe_read_discard(g_e1[chan].tx.fifo);
		g_e1[chan].tx.in_flight--;
	}

	/* Recover any done RX BD */
	while ( (bd = e1_regs[chan].rx.bd) & E1_BD_VALID ) {
		/* FIXME: CRC status ? */
		e1f_multiframe_write_commit(g_e1[chan].rx.fifo);
		if ((bd & (E1_BD_CRC0 | E1_BD_CRC1)) != (E1_BD_CRC0 | E1_BD_CRC1))
			printf("b: %03x\n", bd);
		g_e1[chan].rx.in_flight--;
	}

	/* Boot procedure */
#if 0
	if (g_e1[chan].tx.state == BOOT) {
		if (e1f_unseen_frames(g_e1[chan].tx.fifo) < (16 * 5))
			return;
	...
	/* Handle RX */
		/* Misalign ? */
	if (g_e1[chan].rx.state == RUN) {
		if (!(e1_regs[chan].rx.csr & E1_RX_SR_ALIGNED)) {
			printf("[!] E1 rx misalign\n");
			g_e1[chan].rx.state = RECOVER;
		}
	}

		/* Overflow ? */
	if (g_e1[chan].rx.state == RUN) {
		if (e1_regs[chan].rx.csr & E1_RX_SR_OVFL) {
		...
		/* Recover ready ? */
	if (g_e1[chan].rx.state == RECOVER) {
		if (g_e1[chan].rx.in_flight != 0)
			goto done_rx;
		e1f_multiframe_empty(g_e1[chan].rx.fifo);
	}

		/* Fill new RX BD */
	while (g_e1[chan].rx.in_flight < 4) {
		if (!e1f_multiframe_write_prepare(g_e1[chan].rx.fifo, &ofs))
			break;
		e1_regs[chan].rx.bd = e1f_ofs_to_mf(ofs);
		g_e1[chan].rx.in_flight++;
	}

		/* Clear overflow if needed */
	if (g_e1[chan].rx.state != RUN) {
		e1_regs[chan].rx.csr = g_e1[chan].rx.cr | E1_RX_CR_OVFL_CLR;
		g_e1[chan].rx.state = RUN;
	}
done_rx:

	/* Handle TX */
		/* Underflow ? */
	if (g_e1[chan].tx.state == RUN) {
		if (e1_regs[chan].tx.csr & E1_TX_SR_UNFL) {
			printf("[!] E1 underflow %d\n", g_e1[chan].tx.in_flight);
			g_e1[chan].tx.state = RECOVER;
		}
	}

		/* Recover ready ? */
	if (g_e1[chan].tx.state == RECOVER) {
		if (e1f_unseen_frames(g_e1[chan].tx.fifo) < (16 * 5))
			return;
	}

		/* Fill new TX BD */
	while (g_e1[chan].tx.in_flight < 4) {
		if (!e1f_multiframe_read_peek(g_e1[chan].tx.fifo, &ofs))
			break;
		e1_regs[chan].tx.bd = e1f_ofs_to_mf(ofs);
		g_e1[chan].tx.in_flight++;
	}

		/* Clear underflow if needed */
	if (g_e1[chan].tx.state != RUN) {
		e1_regs[chan].tx.csr = g_e1[chan].tx.cr | E1_TX_CR_UNFL_CLR;
		g_e1[chan].tx.state = RUN;
	}
}

void
e1_debug_print(int chan, bool data)
{
	volatile uint8_t *p;

	puts("E1\n");
	printf("CSR: Rx %04x / Tx %04x\n", e1_regs[chan].rx.csr, e1_regs[chan].tx.csr);
	printf("InF: Rx %d / Tx %d\n", g_e1[chan].rx.in_flight, g_e1[chan].tx.in_flight);
	printf("Sta: Rx %d / Tx %d\n", g_e1[chan].rx.state, g_e1[chan].tx.state);

	e1f_debug(g_e1[chan].rx.fifo, "Rx FIFO");
	e1f_debug(g_e1[chan].tx.fifo, "Tx FIFO");
